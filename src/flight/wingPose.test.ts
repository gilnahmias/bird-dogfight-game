...
function buildWings() {
  ...
  const make = (mirrored: boolean) => {
    ...
    // The wrist carries the whole hand, pivoting on its leading edge, exactly as
    // Bird.tsx assembles it.
    const wrist = new Group()
    wrist.position.set(0, 0, -0.74)
    const manus = new Group()
    manus.position.set(0, 0, 0.74)
    const tip = new Group()
    tip.position.set(3.2, 0, 0)
    manus.add(tip)
    wrist.add(manus)
    elbow.add(wrist)
    shoulder.add(elbow)
